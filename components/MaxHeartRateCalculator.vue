<template>
    <div class="bg-white py-8 px-4 border-2 border-gray-950 shadow sm:px-10 sm:mx-auto sm:w-full sm:max-w-md">
        <div class="flex flex-col gap-4">
            <h2 class="text-xl text-gray-900 font-bold">Maximum Heart Rate Calculator</h2>
      <div>
      ...
      <button @click="calculateMHR" class="rounded-full bg-gray-700 py-2 px-4 text-white hover:bg-gray-950 cursor-pointer font-bold">Calculate Maximum Heart Rate</button>
      <div v-if="mhr" class="flex flex-col gap-2">
        <div>
          Your estimated Maximum Heart Rate is: 
          <div class="text-3xl font-bold">{{ mhr }} bpm</div>
        </div>
        <table>
          <thead>
            <tr>
              <th class="text-left">Percentage</th>
              <th class="text-right">Heart Rate (bpm)</th>
            </tr>
          </thead>
          <tbody>
            <tr v-for="(percentage, index) in percentages" :key="index">
              <td>{{ percentage * 100 }}%</td>
              <td class="text-right">{{ Math.round(mhr * percentage) }}</td>
            </tr>
          </tbody>
        </table>
      </div>
  ...
  const age = ref(0);
  const mhr = ref(null);
  const percentages = [0.9, 0.8, 0.7, 0.6, 0.5];
  
  const calculateMHR = () => {
    mhr.value = 220 - age.value;
  };
  </script>
