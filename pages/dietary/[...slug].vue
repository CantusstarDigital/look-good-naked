<template>
    ...
    <header v-if="data.article" class="max-w-7xl mx-auto px-6 md:px-8 py-20">
        <div class="max-w-4xl flex flex-col gap-3">
          ...
          <div class="space-x-2">
            <NuxtLink :to="`/dietary/tags/${tag}`" v-for="(tag, n) in data.article.tags" :key="n" class="group inline-flex items-center h-6 rounded-full text-sm font-semibold whitespace-nowrap px-3 focus:outline-none focus:ring-2 bg-slate-100 text-slate-700 hover:bg-slate-200 hover:text-gray-900 focus:ring-gray-500">{{ tag }}</NuxtLink>
          </div>
        </div>
    </header>
          ...
          <ol role="list" class="flex items-center space-x-1">
            ...
            <li>
              <div class="flex items-center">
                ...
                <NuxtLink to="/dietary" class="ml-1 text-sm font-bold text-gray-500 hover:text-gray-950">Dietary</NuxtLink>
              </div>
            </li>
          </ol>
        </nav>
