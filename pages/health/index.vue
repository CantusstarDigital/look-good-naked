                    ...
                    <div class="flex flex-col gap-5">
                        ...
                        <div class="space-x-2">
                            <span v-for="(tag, n) in articleTags" :key="n">
                                <NuxtLink :to="`/health/tags/${tag}`" class="group font-hand inline-flex items-center h-6 rounded-full text-sm whitespace-nowrap px-3 focus:outline-none focus:ring-2 bg-slate-100 text-slate-700 hover:bg-slate-200 hover:text-slate-900 focus:ring-slate-500">{{ tag }}</NuxtLink>
                            </span>
                        </div>
                    </div>
                </div>
