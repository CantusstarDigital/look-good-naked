    ...
    <header v-if="data.article" class="container mx-auto px-6 md:px-8 py-20">
        <div class="max-w-4xl">
          ...
          <div class="mt-4 space-x-2">
            <NuxtLink :to="`/lifestyle/tags/${tag}`" v-for="(tag, n) in data.article.tags" :key="n" class="group inline-flex items-center h-6 rounded-full text-sm font-semibold whitespace-nowrap px-3 focus:outline-none focus:ring-2 bg-slate-100 text-slate-700 hover:bg-slate-200 hover:text-slate-900 focus:ring-slate-500">{{ tag }}</NuxtLink>
          </div>
        </div>
    </header>
